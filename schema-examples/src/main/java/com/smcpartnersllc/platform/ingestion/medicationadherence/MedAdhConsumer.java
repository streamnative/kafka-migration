...
import io.streamnative.avro.DataRecordAvro;
...
public class MedAdhConsumer {
    ...
    public static void main(String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        final Consumer<String, MedicationAdherenceRecord> consumer = new KafkaConsumer<String, MedicationAdherenceRecord>(props);
        consumer.subscribe(Arrays.asList(topic));

        try {
            while (true) {
                ConsumerRecords<String, MedicationAdherenceRecord> records = consumer.poll(100);
                for (ConsumerRecord<String, MedicationAdherenceRecord> record : records) {
                    String key = record.key();
                    MedicationAdherenceRecord value = record.value();
